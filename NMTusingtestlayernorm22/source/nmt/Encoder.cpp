...
XTensor AttEncoder::Make_int8(XTensor& input, XTensor* mask, XTensor& maskEncDec, bool isTraining)
{
    XTensor x;

    x = embedder.Make_int8(input, false, isTraining);
    ...
    for (int i = 0; i < nlayer; i++) {
        ...
        attnBefore = LayerNorm(x, attLayerNorms[i], preNorm, true, false);
        /* self attention */
        att = selfAtt[i].Make_int8(attnBefore, attnBefore, attnBefore, mask, isTraining, NULL, SELF_ATT);
...
XTensor AttEncoder::Make_int8(XTensor& input, XTensor* mask, bool isTraining)
{
    XTensor nothing;

    return Make_int8(input, mask, nothing, isTraining);
}
