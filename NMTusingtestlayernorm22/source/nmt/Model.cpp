...
void Model::InitModel(Config& config)
 {
    ...
    FILE* modelFile = NULL;
    initint8(config);
    //FILE* modelfile2 = fopen("converted", "wb");
    //Dump("converted");
...
void Model::initint8(Config &config) {
    ifstream* fp = new ifstream(config.testFN);
    int num = 0;
    string line;
...
void Model::ReadandConvert(FILE* file)
{
    double startT = GetClockSec();
    FILE* modelFile2 = fopen("CONVERTED", "wb");
    TensorList params;
    ...
    for (int i = 0; i < params.Size(); i++)
    {
        ...
        if (i == 256||i==257)
            continue;
        quantize(params[i]);
        params[i]->BinaryDump(modelFile2);
    }
        /* share all embeddings */
    if (shareAllEmbeddings == 1) {
        _CopyValues(&encoder->embedder.w, &decoder->embedder.w);
