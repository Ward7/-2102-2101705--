...
XTensor Embedder::Make(XTensor& input, bool isDec, bool isTraining, int nstep)
{
    ...
    //LOG("quantize wordEmbedding elapsed %lf", programTimes);
    ...
    //LOG("linear wordEmbedding elapsed %lf", programTimes1);
    ...
    //LOG("sum wordembedding and posembedding elapsed %lf", programTimes2);
    return wordEmbedding;
}


XTensor Embedder::Make_int8(XTensor& input, bool isDec, bool isTraining, int nstep)
{
    /* make sure the padding index is 1 */
    CheckNTErrors(input.order > 1, "Wrong input tensor size!");
    CheckNTErrors(input.dimSize[input.order - 1] < maxLength, "The sequence is too long!");
