    ...
    bool isTraining, Cache* cache, int attType)
{
    ...
    if (!cache || isTraining || !(cache->enable)) {
        ...
        //nmt::quantize(&biasK);
        //nmt::quantize(&weightK);
        //nmt::quantize(&biasV);
        //nmt::quantize(&weightV);
        ...
        if (attType == SELF_ATT) {
            if(k.dataType!=X_INT8)
            quantize(&k);
            if (v.dataType != X_INT8)
            quantize(&v);
            quantize(&biasK);
            quantize(&weightK);
            quantize(&biasV);
            ...
            if (!cache->miss) {
                if (cache->key.dataType != X_INT8)
                    quantize(&cache->key);
                if (cache->value.dataType != X_INT8)
                    quantize(&cache->value);
                k2 = Concatenate(cache->key, k2, 1);
                v2 = Concatenate(cache->value, v2, 1);
                dequantize(&cache->key);
                dequantize(&cache->value);
            }
            dequantize(&k2);
            dequantize(&v2);
            cache->key = k2;
...
XTensor Attention::Make_int8(XTensor& k, XTensor& q, XTensor& v, XTensor* mask,
    bool isTraining, Cache* cache, int attType)
{
    const bool isEnc = (!cache) ? true : false;
    ...
    if (!cache || isTraining || !(cache->enable)) {
        ...
        if (useRPR && attType == SELF_ATT)
            return MakeRPRAttention(k2, q2, v2, mask, isTraining, isEnc);
        return MakeAttention_int8(k2, q2, v2, mask, isTraining);
    }
        ...
        if (attType == SELF_ATT) {
            ...
            if (useRPR)
                return MakeRPRAttention(cache->key, q2, cache->value, mask, isTraining, isEnc);
            return MakeAttention_int8(cache->key, q2, cache->value, mask, isTraining);
        }
        else if (attType == EN_DE_ATT) {
            if (cache->miss) {
                cache->key = MulAndShift(k, weightK, biasK);
    ...
    XTensor* mask, bool isTraining)
{
   ...
   // nmt::quantize(&k);
    ...
    if (dot.dataType == X_INT8) {
        dequantize(&dot);
    }
    //dequantize(&kheads);
    //dequantize(&qheads);
    if (vheads.dataType == X_INT8) {
        dequantize(&vheads);
    }
    ...
    //if (weightO.dataType != X_INT8) {
     //   quantize(&weightO);
     //   quantize(&biasO);
   // }
    
    //quantize(&att);
    XTensor atted = Merge(att, att.order - 1);
    //dequantize(&atted);
    return MulAndShift(atted, weightO, biasO);
}

XTensor Attention::MakeAttention_int8(XTensor& k, XTensor& q, XTensor& v,
    XTensor* mask, bool isTraining)
{
    XTensor kheads;
    XTensor qheads;
