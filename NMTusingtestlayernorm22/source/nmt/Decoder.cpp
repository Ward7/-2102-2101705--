                         ...
                         XTensor* maskEncDec, int nstep, bool isTraining)
{
    ...
    if (preNorm)
        if (x.dataType == X_INT8) {
            return decoderLayerNorm->Make_int8(x);
        }
        else {
...
XTensor AttDecoder::Make_int8(XTensor& inputDec, XTensor& outputEnc, XTensor* mask,
    XTensor* maskEncDec, int nstep, bool isTraining)
{
    XTensor x;

    x = embedder.Make_int8(inputDec, true, isTraining, nstep);
    ...
    for (int i = 0; i < nlayer; i++) {
        ...
        /******************/
        /* self attention */
        att = selfAtt[i].Make_int8(selfAttnBefore, selfAttnBefore, selfAttnBefore,
            mask, isTraining, &selfAttCache[i], SELF_ATT);
        ...
        /* encoder-decoder attention */
        ende = enDeAtt[i].Make_int8(outputEnc, endeAttnBefore, outputEnc, maskEncDec,
            isTraining, &enDeAttCache[i], EN_DE_ATT);
